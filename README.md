...
- Most of the frontend is directly borrowed from [openmm-torch](`https://github.com/openmm/openmm-torch.git`) with the following main changes:
    - `JaxForce` expects separate functions for `energy`, `forces` and `energy + forces` to export instead of relying on a general `energy + forces` function. This saves up compute time when only OpenMM requests only energy or forces.
