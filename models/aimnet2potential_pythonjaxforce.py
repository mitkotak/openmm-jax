...
class AIMNet2PythonJaxForcePotentialImplFactory(MLPotentialImplFactory):
    def createImpl(
        self,
        name,
        modelPath=None,
        ...
        **args,
        ...
        return AIMNet2PythonJaxForcePotentialImpl(
            name,
            modelPath=modelPath,
            charge=charge,
...
class AIMNet2PythonJaxForcePotentialImpl(MLPotentialImpl):
    def __init__(
        self,
        name,
        modelPath=None,
    ...
    def addForces(
        ...
        **args,
        ...
        if atoms is not None:
            raise ValueError("AIMNet2 PythonJaxForce does not support atom subsets")
        ...
        if model_ref is None:
            if self.name in AIMNET2_MODEL_NAMES:
                ...
                raise ValueError(
                    "modelPath must be provided for custom AIMNet2 PythonJaxForce models"
                )
        model = load_model(
            model_ref,
        ...
        if preprocessing_positions is None:
            raise ValueError("AIMNet2 PythonJaxForce requires preprocessing_positions.")
        ...
        compute = _ComputeAIMNet2PythonJaxForce(
            model=model,
            species=species,
...
for model_name in AIMNET2_MODEL_NAMES:
    MLPotential.registerImplFactory(
        f"{model_name}-pythonjaxforce", AIMNet2PythonJaxForcePotentialImplFactory()
    )

__all__ = [
    "MLPotential",
    "AIMNet2PythonJaxForcePotentialImplFactory",
    "AIMNet2PythonJaxForcePotentialImpl",
]
...
class _ComputeAIMNet2PythonJaxForce:
    def __init__(
        self,
        *,
        model,
