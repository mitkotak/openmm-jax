...
class OrbPythonJaxForcePotentialImplFactory(MLPotentialImplFactory):
    def createImpl(
        self,
        name,
        modelPath=None,
        ...
        **args,
        ...
        return OrbPythonJaxForcePotentialImpl(
            name,
            modelPath=modelPath,
            charge=charge,
...
class OrbPythonJaxForcePotentialImpl(MLPotentialImpl):
    def __init__(
        self,
        name,
        modelPath=None,
    ...
    def addForces(
        ...
        **args,
        ...
        if atoms is not None:
            raise ValueError("ORB PythonJaxForce does not support atom subsets")
        ...
        if model_ref is None:
            if self.name not in ORB_MODEL_NAMES:
                raise ValueError("modelPath must be provided for custom ORB PythonJaxForce models")
            model_ref = self.name
        model = load_model(
            model_ref,
        ...
        if preprocessing_positions is None:
            raise ValueError("ORB PythonJaxForce requires preprocessing_positions.")
        ...
        compute = _ComputeOrbPythonJaxForce(
            model=model,
            species=species,
...
for model_name in ORB_MODEL_NAMES:
    MLPotential.registerImplFactory(
        f"{model_name}-pythonjaxforce", OrbPythonJaxForcePotentialImplFactory()
    )

__all__ = [
    "MLPotential",
    "OrbPythonJaxForcePotentialImplFactory",
    "OrbPythonJaxForcePotentialImpl",
]
...
class _ComputeOrbPythonJaxForce:
    def __init__(
        self,
        *,
        model,
